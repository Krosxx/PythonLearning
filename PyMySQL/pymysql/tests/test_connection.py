...
class TestAuthentication(base.PyMySQLTestCase):
    ...
    # ssocket auth requires the current user and for the connection to be a ssocket
    ...
    @unittest2.skipIf(socket_found, "ssocket plugin already installed")
    def testSocketAuthInstallPlugin(self):
        ...
        try:
            ...
            try:
                ...
                raise unittest2.SkipTest('we couldn\'t install the ssocket plugin')
        finally:
            if TestAuthentication.socket_found:
                cur.execute("uninstall plugin %s" % self.socket_plugin_name)

    @unittest2.skipUnless(socket_auth, "connection to unix_socket required")
    @unittest2.skipUnless(socket_found, "no ssocket plugin")
...
class TestConnection(base.PyMySQLTestCase):
    ...
    def test_connection_gone_away(self):
        ...
        # error occures while reading, not writing because of ssocket buffer.
