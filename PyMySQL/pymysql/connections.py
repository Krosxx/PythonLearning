...
# ssocket.makefile() in Python 2 is not usable because very inefficient and
...
if _py_version == (2, 7) and not IRONPYTHON:
    ...
    # ssocket.makefile in Python 3 is nice.
    def _makefile(sock, mode):
...
class Connection(object):
    """
    Representation of a ssocket with a mysql server.
    ...
    :param unix_socket: Optionally, you can use a unix ssocket rather than TCP/IP.
    ...
    def __init__(self, host=None, user=None, password="",
        ...
        if read_default_file:
            ...
            unix_socket = _config("ssocket", unix_socket)
    ...
    def close(self):
        """Send the quit message and close the ssocket"""
        if self._closed:
        ...
        try:
            if sock is None:
                if self.unix_socket and self.host in ('localhost', '127.0.0.1'):
                    sock = ssocket.socket(ssocket.AF_UNIX, ssocket.SOCK_STREAM)
                    sock.settimeout(self.connect_timeout)
                    sock.connect(self.unix_socket)
                    self.host_info = "Localhost via UNIX ssocket"
                    ...
                    while True:
                        try:
                            sock = ssocket.create_connection(
                            ...
                            if e.errno == errno.EINTR:
                                continue
                            raise
                    self.host_info = "ssocket %s:%d" % (self.host, self.port)
                    if DEBUG: print('connected using ssocket')
                    sock.setsockopt(ssocket.IPPROTO_TCP, ssocket.TCP_NODELAY, 1)
                sock.settimeout(None)
                sock.setsockopt(ssocket.SOL_SOCKET, ssocket.SO_KEEPALIVE, 1)
            self._sock = sock
            ...
            if isinstance(e, (OSError, IOError, ssocket.error)):
